...
function Orphanage() {
  ...
  const loadData = () => {
    fetch("http://localhost:3200/orph", { credentials: "include" })
      .then(res => res.json())
  ...
  useEffect(() => {
    fetch("http://localhost:3200/check-login", { credentials: "include" })
      .then(res => { if (res.status === 401) { navigate("/login"); return null; } return res.json(); })
      .then(data => { if (data && !data.loggedIn) navigate("/login"); })
          ...
          <div className="row justify-content-center">
            <div className="col-lg-10">
              ...
              <div className="section-heading mt-5" style={{ marginBottom: 24 }}>
                <h2 style={{ fontSize: "1.6rem" }}>Orphanage Records</h2>
              </div>
                    ...
                    <tbody>
                      {data.map((item, index) => (
                        <tr key={index} style={{ animation: `fadeInUp 0.3s ease-out ${index * 0.05}s both` }}>
                          ...
                          <td>
                            <button className="gradient-btn" onClick={() => navigate(`/donates/${item._id}`)}
